...
public class PlayerClass : MonoBehaviour
{
    ...
    private void Update()
    {
        switch (SelectedClass)
        {
            ...
            case CaretakerClass.Assistant:
                if (Input.GetButtonDown("CaretakerSpecificAction")) ActivateCCTV();

                if (Input.GetKeyDown(KeyCode.Tab)) ResetCam();
                break;
            default:
    ...
    private void ActivateCCTV()
    {
        ...
        if (message.ToLower().Contains("northwest"))
        {
            ...
            camIndex++;
            if (camIndex < _cctvCamsNW.Length) _cctvCamsNW[camIndex].Priority = 1;
            if (camIndex > _cctvCamsNW.Length - 1) ResetCam();
        }
    }
    ...
    {
        minimapIndicatior.GetComponent<MinimapIndicator>().RadarPing();
